...
// Send a raw custom message to any number (used for owner notifications)
export async function sendRawSMS({ phone, message }) {
  try {
    const apiKey = process.env.FAST2SMS_API_KEY;
    if (!apiKey) {
      console.warn('FAST2SMS_API_KEY not set — SMS skipped.');
    ...
    const result = await response.json();
    console.log('Owner SMS raw response:', JSON.stringify(result));
    return result.return === true
      ? { success: true }
      : { success: false, error: result.message };
  } catch (err) {
    console.error('sendRawSMS error:', err.message);
    return { success: false, error: err.message };
  }
}
